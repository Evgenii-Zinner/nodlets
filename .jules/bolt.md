...
## 2026-03-07 - DOM Lookups in the Update Loop
**Learning:** Found multiple instances of `document.getElementById` inside the high-frequency `update(deltaTime)` loop and randomly triggered UI update methods (`updateCounts`, `updateGlobalStats`, etc.) in `game.js`. Querying the DOM is synchronously blocking and expensive compared to JS logic, especially when done 60 times per second for an element that never changes identity (`upgradeBtn`).
**Action:** Pre-cache DOM elements in the constructor or `init()` phase. Use standard `this.ui.upgradeBtn = document.getElementById(...)` references to turn expensive O(DOM_depth) string lookups into instantaneous O(1) object references.
