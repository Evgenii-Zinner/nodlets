            ...
            if (state === 0 || state === 2) { // Seeking or Orbiting Target Server
                ...
                if (targetId !== -1) {
                    ...
                    if (distSq > orbitRadius * orbitRadius * 1.5) {
                        ...
                        // ⚡ Bolt Optimization: Multiply hoisted reciprocal instead of 2 divisions
                        ...
                        // ⚡ Bolt Optimization: Hoist 1/dist for pull/tangent calc to save divisions
                        const invDist = 1 / dist;
                        const pullFactorOverDist = ((dist - orbitRadius) * 0.1) * invDist;
                        const pullX = dx * pullFactorOverDist;
                        const pullY = dy * pullFactorOverDist;

                        // Orbit speed (significantly faster as requested)
                        const orbitSpeedOverDist = orbitSpeed * invDist;
                        const tangentX = pX * orbitSpeedOverDist;
                        const tangentY = pY * orbitSpeedOverDist;
                ...
                if (distSq < this.hubs.size[hubIdx] * this.hubs.size[hubIdx]) {
                    ...
                    // ⚡ Bolt Optimization: Multiply hoisted reciprocal
